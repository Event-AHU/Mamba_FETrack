...
from lib.train.actors import Mamba_FETrackActor
# for import modules
import importlib
...
def run(settings):
    ...
    # Create network
    if settings.script_name == "mamba_fetrack":
        net = build_mamba_fetrack(cfg)
    else:
    ...
    if settings.script_name == "mamba_fetrack":
        ...
        actor = Mamba_FETrackActor(net=net, objective=objective, loss_weight=loss_weight, settings=settings, cfg=cfg)
    else:
        raise ValueError("illegal script name")
