...
class MambaInnerFnNoOutProj(torch.autograd.Function):
    ...
    def forward(ctx, xz, conv1d_weight, conv1d_bias, x_proj_weight, delta_proj_weight,
                A, B=None, C=None, prompt=None, D=None, delta_bias=None, B_proj_bias=None,
        ...
        x, z = xz.chunk(2, dim=1)                  # xz = [b, 1536, 320]         x = [B, 768, 320]          z = [B, 768, 320] 
        ...
        prompt = prompt.transpose(1,2).unsqueeze(1)
        C = C + prompt
        ...
        ctx.prompt_needs_grad = prompt is not None and prompt.requires_grad
        if checkpoint_lvl >= 1:  # Will recompute conv1d_out and delta in the backward pass
            conv1d_out, delta = None, None

        ctx.save_for_backward(xz, conv1d_weight, conv1d_bias, x_dbl, x_proj_weight,
                              delta_proj_weight, conv1d_out, delta,
                              A, B, C, prompt, D, delta_bias, scan_intermediates, out)
    ...
    def backward(ctx, dout):
        ...
        (xz, conv1d_weight, conv1d_bias, x_dbl, x_proj_weight, delta_proj_weight, 
         conv1d_out, delta, A, B, C, prompt, D, delta_bias, scan_intermediates, out) = ctx.saved_tensors
        L = xz.shape[-1]
        delta_rank = delta_proj_weight.shape[1]
        d_state = A.shape[-1] * (1 if not A.is_complex() else 2)
        ...
        # Handle prompt gradient if needed
        dprompt = None
        if ctx.prompt_needs_grad:
            dprompt = dC  # dC contains the gradient from the prompt addition
            dprompt = dprompt.sum(dim=1)  
            dprompt = dprompt.transpose(1, 2)  # Reverse the transpose from forward
...
def mamba_inner_fn_no_out_proj(
    ...
    return MambaInnerFnNoOutProj.apply(xz, conv1d_weight, conv1d_bias, x_proj_weight, delta_proj_weight,
                              A, B, C, prompt, D, delta_bias, B_proj_bias, C_proj_bias, delta_softplus)
