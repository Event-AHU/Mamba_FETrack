...
class Block(nn.Module):
    ...
    def forward(
        self, hidden_states: Tensor, prompt, residual: Optional[Tensor] = None, inference_params=None
    ):
        ...
        hidden_states = self.mixer(hidden_states, prompt, inference_params=inference_params)
...
class VisionMamba(nn.Module):
    def __init__(self, 
        ...
        self.rgb_specific_prompt = nn.Embedding(self.embed_dim // 2, 16)
        self.event_specific_prompt = nn.Embedding(self.embed_dim // 2, 16)
        
        torch.nn.init.xavier_uniform_(self.rgb_specific_prompt.weight)
        torch.nn.init.xavier_uniform_(self.event_specific_prompt.weight)
    ...
    def forward_features(self, rgb_z, rgb_x, event_z, event_x, inference_params=None, if_random_cls_token_position=False, if_random_token_rank=False):
        # taken from https://github.com/rwightman/pytorch-image-models/blob/master/timm/models/vision_transformer.py
        # with slight modifications to add the dist_token
        rgb_z = self.patch_embed(rgb_z)                  #x.shape = torch.Size([B, 3, 256, 256])  -> torch.Size([2, 256, 384])
        rgb_x = self.patch_embed(rgb_x)                  #z.shape = torch.Size([B, 3, 128, 128])  -> torch.Size([2, 64, 384])
        event_z = self.patch_embed(event_z)                
        event_x = self.patch_embed(event_x)
    
        if self.if_abs_pos_embed:                  # True 
            rgb_x = rgb_x + self.pos_embed_x               # x = x + positon_embemding =torch.Size([B, 256, 384]) + torch.Size([1, 256, 384]) = torch.Size([B, 256, 384])
            rgb_z = rgb_z + self.pos_embed_z               # z = z + positon_embemding =torch.Size([B, 64, 384]) + torch.Size([1, 64, 384]) = torch.Size([B, 64, 384])
            event_x = event_x + self.pos_embed_x           # x = x + positon_embemding =torch.Size([B, 256, 384]) + torch.Size([1, 256, 384]) = torch.Size([B, 256, 384])
            event_z = event_z + self.pos_embed_z           # z = z + positon_embemding =torch.Size([B, 64, 384]) + torch.Size([1, 64, 384]) = torch.Size([B, 64, 384])
        
        rgb_tokens = torch.cat((rgb_z, rgb_x), dim=1)
        event_tokens = torch.cat((event_z, event_x), dim=1)
        
        rgb_onehot = F.gumbel_softmax(self.route(rgb_tokens), hard=True, dim=-1)
        event_onehot = F.gumbel_softmax(self.route(event_tokens), hard=True, dim=-1) 
        
        rgbe_prompt_pool = self.rgb_specific_prompt.weight *  self.event_specific_prompt.weight
        
        p_r = rgb_onehot @ rgbe_prompt_pool
        p_e = event_onehot @ rgbe_prompt_pool
        
        p_rgbe = torch.cat((p_r, p_e), dim=1)

        x = torch.cat((rgb_tokens, event_tokens), dim=1)           # torch.Size([B, 640, 384])
        x = self.pos_drop(x)                                             # x.shape = torch.Size([B, 640, 384])
        ...
        if not self.if_bidirectional:                             
            for layer in self.layers:
                hidden_states, residual = layer(
                    hidden_states, p_rgbe, residual, inference_params=inference_params
                )
    ...
    def forward(self, rgb_z, rgb_x,  event_z, event_x, inference_params=None, if_random_cls_token_position=False, if_random_token_rank=False):
        x = self.forward_features(rgb_z, rgb_x,  event_z, event_x, 
                                inference_params, if_random_cls_token_position=if_random_cls_token_position, if_random_token_rank=if_random_token_rank)
        return x
